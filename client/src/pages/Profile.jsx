...
const ProfilePage = () => {
  const { user } = authStore();
    ...
    const bookMarkedContests =  allContests.filter(contest => bookmarks.includes(contest.contestId));
    ...
    <div className="max-w-4xl min-h-screen mx-auto p-6 mt-16">
      <div className="bg-base-200 rounded-lg shadow-md p-6 mb-8 flex items-center">
        ...
        <div className="flex-grow">
          ...
          <p className="mt-2">
            <span className="font-semibold">Bookmarks:</span> {bookmarks?.length || 0}
          </p>
        </div>
      </div>

      <div>
        <h2 className="text-3xl font-bold mb-6 ring-1 rounded-2xl text-center p-2">Bookmarks</h2>
        ...
        {bookMarkedContests && bookMarkedContests.length > 0 ? (
          ...
          <div className="rounded-lg p-8 text-center" onClick={()=>navigate('/')}>
            <p className="">No bookmarks found</p>
            <button href='/' className="mt-4 bg-blue-500 px-4 py-2 rounded cursor-pointer hover:bg-blue-600">
